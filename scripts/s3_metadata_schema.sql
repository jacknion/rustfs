...
-- Schema：使用独立的 rustfs schema 进行命名空间隔离
-- =====================================================

-- =====================================================
-- 创建 RustFS Schema（如果不存在）
-- =====================================================
CREATE SCHEMA IF NOT EXISTS rustfs;

-- 设置搜索路径，优先使用 rustfs schema
SET search_path TO rustfs, public;

-- 删除已存在的表（谨慎使用！）
-- DROP TABLE IF EXISTS rustfs.s3_objects CASCADE;
-- DROP TABLE IF EXISTS rustfs.bucket_stats CASCADE;
-- DROP TABLE IF EXISTS rustfs.tag_statistics CASCADE;

-- =====================================================
-- 主表：S3 对象元数据
-- =====================================================
CREATE TABLE IF NOT EXISTS rustfs.s3_objects (
    -- 主键
...
CREATE INDEX IF NOT EXISTS idx_bucket_key 
    ON rustfs.s3_objects (bucket, object_key);
...
CREATE INDEX IF NOT EXISTS idx_user_metadata_gin 
    ON rustfs.s3_objects USING GIN (user_metadata);

-- 4. 时间范围查询
CREATE INDEX IF NOT EXISTS idx_last_modified 
    ON rustfs.s3_objects (last_modified DESC);

CREATE INDEX IF NOT EXISTS idx_created_at 
    ON rustfs.s3_objects (created_at DESC);

-- 5. 存储类型查询
CREATE INDEX IF NOT EXISTS idx_bucket_storage_class 
    ON rustfs.s3_objects (bucket, storage_class);

-- 6. 合规查询（带条件索引，节省空间）
CREATE INDEX IF NOT EXISTS idx_retention 
    ON rustfs.s3_objects (retention_until) 
...
CREATE TRIGGER s3_objects_updated_at_trigger
    BEFORE UPDATE ON rustfs.s3_objects
    FOR EACH ROW
...
-- 辅助表：Bucket 统计信息（缓存表）
-- =====================================================
CREATE TABLE IF NOT EXISTS rustfs.bucket_stats (
...
BEGIN
    IF TG_OP = 'INSERT' THEN
        INSERT INTO rustfs.bucket_stats (bucket, object_count, total_size_bytes)
        VALUES (NEW.bucket, 1, NEW.size_bytes)
        ON CONFLICT (bucket) DO UPDATE SET
            object_count = rustfs.bucket_stats.object_count + 1,
            total_size_bytes = rustfs.bucket_stats.total_size_bytes + NEW.size_bytes,
            last_updated = NOW();
    ELSIF TG_OP = 'DELETE' THEN
        UPDATE rustfs.bucket_stats SET
...
--     AFTER INSERT OR UPDATE OR DELETE ON rustfs.s3_objects
--     FOR EACH ROW
...
-- 辅助表：标签使用统计
-- =====================================================
CREATE TABLE IF NOT EXISTS rustfs.tag_statistics (
...
CREATE INDEX IF NOT EXISTS idx_tag_usage 
    ON rustfs.tag_statistics (usage_count DESC);
...
-- 视图 1：最新对象（按 bucket）
CREATE OR REPLACE VIEW rustfs.recent_objects AS
SELECT 
    bucket,
    object_key,
    size_bytes,
...
-- 视图 2：大文件对象（> 100MB）
CREATE OR REPLACE VIEW rustfs.large_objects AS
SELECT 
    bucket,
    object_key,
    size_bytes,
...
-- 视图 3：加密对象统计
CREATE OR REPLACE VIEW rustfs.encrypted_objects_summary AS
SELECT 
    bucket,
...
-- 示例数据（测试用）
-- =====================================================
INSERT INTO rustfs.s3_objects (bucket, object_key, size_bytes, last_modified, etag, content_type, tags, user_metadata)
VALUES 
    (
        'example-bucket', 
...
-- 重建索引（如果性能下降）
-- REINDEX TABLE rustfs.s3_objects;
...
-- 授予应用用户权限
GRANT USAGE ON SCHEMA rustfs TO rustfs_user;
GRANT SELECT, INSERT, UPDATE, DELETE ON rustfs.s3_objects TO rustfs_user;
GRANT SELECT, INSERT, UPDATE, DELETE ON rustfs.bucket_stats TO rustfs_user;
GRANT SELECT, INSERT, UPDATE, DELETE ON rustfs.tag_statistics TO rustfs_user;
GRANT USAGE, SELECT ON SEQUENCE rustfs.s3_objects_id_seq TO rustfs_user;

-- 只读用户（用于分析查询）
-- CREATE USER rustfs_readonly WITH PASSWORD 'readonly_password';
-- GRANT CONNECT ON DATABASE rustfs_db TO rustfs_readonly;
-- GRANT USAGE ON SCHEMA rustfs TO rustfs_readonly;
-- GRANT SELECT ON ALL TABLES IN SCHEMA rustfs TO rustfs_readonly;
...
BEGIN
    ...
    RAISE NOTICE '========================================';
    RAISE NOTICE '已创建 schema：rustfs';
    RAISE NOTICE '已创建表：';
    RAISE NOTICE '  - rustfs.s3_objects (主表)';
    RAISE NOTICE '  - rustfs.bucket_stats (统计表)';
    RAISE NOTICE '  - rustfs.tag_statistics (标签统计)';
    RAISE NOTICE '';
